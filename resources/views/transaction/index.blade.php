...
@section('main_content')
    <div id="selling-wrapper">
                ...
                <div class="card card-default" role="tabpanel">
                    ...
                    <div class="tab-content">
                                ...
                                @foreach($transactions as $t)
                                    <tr class="gradeX">
                                        ...
                                        <td>
                                            <div class="row">
                                                <div class="col-md-1">
                                                    <a class="btn btn-xs btn-info" href="{{route('transaction.show', ['id' => $t->id])}}"><em class="fa fa-eye"></em></a>
                                                </div>
                                                <div class="col-md-1">
                                                    <form method="post" action="{{route('transaction.destroy',['id' => $t->id])}}">
                                                        @csrf
                                                        <input type="hidden" name="_method" value="DELETE">
                                                        <button type="submit" class="btn btn-xs btn-info" transaction-id="{{$t->id}}"><em class="fa fa-trash"></em></button>
                                                    </form>
                                                </div>
                                            </div>
                                        </td>
                        ...
                        <div class="tab-pane" id="add-new" role="tabpanel">
                            ...
                            <div class="card-body">
                                <div class="col-md-3">
                                    <input type="hidden" id="client_id" v-model="customer.id">
                                    <h5>Customer : @{{ customer.name }} <button @click="showHidePickCustomer()" class="btn btn-xs btn-info"><em class="fa fa-pencil"></em></button></h5>
                                </div>
                                <div class="col-md-12">
        ...
        <div id="pick-customer" class="my-popup" v-bind:class="{'my-popup-hide':!show_customer_popup, 'my-popup-show':show_customer_popup}">
            <div class="col-md-12">
                <h4>Choose Customer <a class="btn btn-xs pull-right" @click="showHidePickCustomer()"><em class="fa fa-remove"></em></a></h4>
            </div>
            <table class="table table-bordered table-striped data-table-popup">
                ...
                @foreach($customers as $c)
                    <tr>
                        <td><a href="javascript:void(0)" @click="pickCustomer('{{ $c->id }}', '{{ $c->first_name . " " . $c->last_name }}')">{{ $c->id }}</a></td>
                        <td><a href="javascript:void(0)" @click="pickCustomer('{{ $c->id }}', '{{ $c->first_name . " " . $c->last_name }}')">{{ $c->first_name . " " . $c->last_name }}</a></td>
                        <td><a href="javascript:void(0)" @click="pickCustomer('{{ $c->id }}', '{{ $c->first_name . " " . $c->last_name }}')">{{ $c->company_name }}</a></td>
                    </tr>
                @endforeach
                </tbody>
            </table>
...
@section('page_js')
                        ...
                        <style>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-10">
                                    ...
                                    <input type="hidden" id="transaction-category-id" name="transaction_category_id" value="1">
                                </div>
                            </div>
    ...
    @include('transaction.scripts.selling')
@endsection